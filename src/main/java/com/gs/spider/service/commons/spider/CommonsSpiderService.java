...
public class CommonsSpiderService extends AsyncGatherService {
		...
		if (spiderInfo.getDynamicFields() != null) {
			Preconditions.checkArgument(
					...
					spiderInfo.getDynamicFields().stream()
							.filter(fieldConfig -> StringUtils.isBlank(fieldConfig.getName())
									|| (StringUtils.isBlank(fieldConfig.getRegex())
											&& StringUtils.isBlank(fieldConfig.getXpath())))
							.count() == 0,
	 ...
	 *            间隔分钟数
	 */
	public ResultBundle<String> createQuartzJob(String spiderInfoId, int interval) {
		...
		quartzManager.addJobMinutely(spiderInfo.getId(), QUARTZ_JOB_GROUP_NAME,
				String.valueOf(interval) + "-" + spiderInfo.getId() + QUARTZ_TRIGGER_NAME_SUFFIX,
				QUARTZ_TRIGGER_GROUP_NAME, WebpageSpiderJob.class, data, interval);
	...
	public String exportQuartz() {
		...
		for (JobKey jobKey : quartzManager.listAll(QUARTZ_JOB_GROUP_NAME)) {
			Pair<JobDetail, Trigger> pair = quartzManager.findInfo(jobKey);
			long hours = ((SimpleTrigger) ((SimpleScheduleBuilder) pair.getRight().getScheduleBuilder()).build())
					.getRepeatInterval() / DateBuilder.MILLISECONDS_IN_HOUR;
