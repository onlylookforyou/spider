...
public class ESClient {
	...
	public Client getClient() {
		...
		try {
			...
			Builder builder = Settings.builder().put("cluster.name", esClusterName);
			if (needCertificate) {
				builder.put("path.home", ".").put("searchguard.ssl.transport.enabled", true)
						.put("searchguard.ssl.transport.keystore_filepath", keyStorePath)
						.put("searchguard.ssl.transport.keystore_password", keyStorePassword)
						.put("searchguard.ssl.transport.truststore_filepath", trustStorePath)
		...
		if (!client.admin().indices().typesExists(new TypesExistsRequest(new String[] { index }, type)).actionGet()
			...
			try {
				putMappingRequest = Requests.putMappingRequest(index).type(type)
						.source(FileUtils.readFileToString(mappingFile));
			} catch (IOException e) {
		...
		if (!client.admin().indices().exists(new IndicesExistsRequest(index)).actionGet().isExists()) {
			...
			try {
				createIndexResponse = client.admin().indices().prepareCreate(index)
						.setSettings(FileUtils.readFileToString(indexMappingFile)).execute().actionGet();
			} catch (IOException e) {
